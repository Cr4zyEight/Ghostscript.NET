...
public class PdfConverter : IDisposable
{
    ...
    protected void WritePdfMark()
    {
        ...
        if (MXmlOutFile != null)
        {
            if (!File.Exists(MXmlOutFile)) throw new Exception("Datei " + MXmlOutFile + " existiert nicht");
        ...
        if (MPdfInFile == MPdfOutFile) throw new Exception("Eingabedatei darf nicht Ausgabedatei sein");
...
<rdf:Description rdf:about="""" xmlns:fx=""urn:factur-x:pdfa:CrossIndustryDocument:invoice:1p0#"">
  ...
  /Ext_Metadata pdfmark";
        ...
        UTF8Encoding utf8 = new(false); // kein BOM verwenden, s. https://docs.microsoft.com/de-de/dotnet/api/system.text.utf8encoding?view=netcore-3.1
    ...
    public bool ConvertToPdfa3(string gsdll)
    {
        ...
        if (!File.Exists(_fileGsdllDll)) throw new Exception("Ghostscript DLL " + _fileGsdllDll + " bitte ins Verzeichnis " + Directory.GetCurrentDirectory() + " kopieren");

        if (!File.Exists(_fileAdobeRgb1998Icc)) throw new Exception("Farbpprofile " + _fileAdobeRgb1998Icc + " bitte ins Verzeichnis " + Directory.GetCurrentDirectory() + " kopieren");
        if (!File.Exists(MPdfInFile)) throw new Exception("Eingabedatei " + MPdfInFile + " existiert nicht");
        if (File.Exists(MPdfOutFile))
        {
            if ((File.GetAttributes(MPdfOutFile) & FileAttributes.ReadOnly) == FileAttributes.ReadOnly) throw new Exception($"Ausgabedatei {MPdfOutFile} keine Schreibrechte");
            FileInfo fi = new(MPdfOutFile);
            if (IsFileLocked(fi)) throw new Exception("Ausgabedatei " + MPdfOutFile + " nicht beschreibbar (noch geffnet?)");
        }
