...
public class ZUGFeRDImporter
{
    ...
    /// </summary>
    private Dictionary<string, sbyte[]> _additionalXmLs = new();

    /// <summary>
    /// Raw XML form of the extracted data - may be directly obtained.
    /// </summary>
    private byte[] _rawXml;
    ...
    public ZUGFeRDImporter(string pdfFilename)
    {
        ...
        if (!File.Exists(pdfFilename)) throw new Exception("File not found");
    ...
    */
    ...
    public virtual string GetForeignReference()
    {
        ...
        if (ReferenceEquals(result, null) || result.Length == 0) result = ExtractString("//*[local-name() = 'ApplicableSupplyChainTradeSettlement']/*[local-name() = 'PaymentReference']");
        return result;
    }
    ...
    {
        if (!ContainsMetaConflict) throw new Exception("Not yet parsed");
        if (GetUtf8().Contains("<rsm:CrossIndustryDocument"))
            return 1;
        if (GetUtf8().Contains("<rsm:CrossIndustryInvoice")) return 2;
        throw new Exception("ZUGFeRD version could not be determined");
    }

    public virtual string GetUtf8()
    {
        if (_rawXml == null) return null;
